    ...
    $scope.editActivity = function (ev, activityIn) {
      ...
      $mdDialog.show({
        ...
        .then(function (activity) {
          // If there is existing calendar data, just push the new activity
          // to the array of activities. Otherwise, set calendar.data to
          // an array containing the activity.
          var action = (
            vm.calendar.data
            ? function() { vm.calendar.data.push(activity); }
            : function() { vm.calendar.data = [activity]; }
          )
          action()
